...
class Customer_profile extends CI_Controller
{
  ...
  public function updateAddress()
  {
    ...
    $checkDataExist = $this->customerProfile_m->existsDataAddress($email);

    if ($checkDataExist == true) {
      $update = $this->customerProfile_m->updateAddress($email, $data);
      ...
      $this->db->insert('customer_address', ['email' => $email, 'province_id' => $this->input->post('province', true), 'province' => $this->input->post('province_name', true), 'city_id' => $this->input->post('regency', true), 'city_name' => $this->input->post('regency_name', true), 'street_name' => $this->input->post('street_name', true)]);

      $response['status'] = "true";
      $response['notif'] = 'Your address has been saved!';
    }
