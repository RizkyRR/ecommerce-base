...
class Company extends CI_Controller
{
  ...
  public function getAllDashRow()
  {
    $data = $this->company_m->getAllDash();
    echo json_encode($data);
  }

  public function getAllIconRow()
  {
    $data = $this->company_m->getFontawesome();
    echo json_encode($data);
  }

  public function getAllColorRow()
  {
    $data = $this->company_m->getColor();
    echo json_encode($data);
  }
  ...
  public function add_dashboard()
  {
    ...
    if ($this->form_validation->run() == TRUE) {
      $data = [
        'detail_dash_id' => $this->input->post('title'),
        'icon_id' => $this->input->post('icon'),
        'color_id' => $this->input->post('color'),
      ];

      $insert = $this->company_m->insertDashboard($data);

      if ($insert > 0) {
        $response['status'] = TRUE;
        $response['notif'] = 'New dashboard info has been added!';
        $response['id'] = $insert;
      } else {
        $response['status'] = FALSE;
  ...
  public function delete_dashboard()
  {
    ...
    $delete = $this->company_m->deleteDashboard($id);
