...
<script type="text/javascript">
  ...
  function show_report_hutang(startDate, endDate) {
    $.ajax({
      url: "<?php echo base_url('report_hutang/getDataHutang') ?>",
      type: "POST",
      async: true,
      ...
      success: function(response) {
        ...
        for (i = 0; i < response.length; i++) {
          html += '<tr>' +
            '<td>' + number + '</td>' +
            '<td>' + response[i].hutang_id + '</td>' +
            '<td>' + response[i].purchase_id + '</td>' +
            '<td>' + moment(response[i].lastupdate).format('DD MMMM YYYY') + '</td>' +
            '<td>' + 'Rp. ' + formatter.format(response[i].total) + '</td>' +
            '<td>' + 'Rp. ' + formatter.format(response[i].remaining) + '</td>' +
            '</tr>'
        }

        $('#table_report_hutang').html(html); */
        ...
        if (response.cetak) {
          ...
          $('#table_report_hutang').html(response.data);
        } else {
  ...
  $('#reservation').daterangepicker({
    ...
    show_report_hutang(startDate, endDate);
  });
</script>